...
# Public web root. Caddy serves from here, so .env, config/ and vendor/ all sit
# one level up and are unreachable over HTTP.
ENV SERVER_ROOT=/var/www/html/htdocs

COPY Caddyfile /etc/caddy/Caddyfile

# Prepares var/ dirs, seeds .env, runs composer install, and sets the opcache
# and Composer flags from ENVIRONMENT in .env. See docker-entrypoint.sh.
